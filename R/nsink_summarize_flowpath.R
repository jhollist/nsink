...
#' @param removal_rasters the removal raster stack, generated by
...
#' }
nsink_summarize_flowpath <- function(flowpath, removal_rasters,
                                     method = c("raster"), filter_window = 3){
  method <- match.arg(method)
  pb <- progress::progress_bar$new(total = 5)
  pb$tick()
  if(method == "raster"){
    fp_removal_raster <- rasterize(flowpath, removal_rasters[[1]], 1, max) *
      removal_rasters[[1]]
    pb$tick()
    fp_type_raster <- rasterize(flowpath, removal_rasters[[2]], 1, max) *
      removal_rasters[[2]]
    pb$tick()
    removal <- extract(removal_rasters[[1]], flowpath, along = TRUE)[[1]]
    pb$tick()
    type <- extract(removal_rasters[[2]], flowpath, along = TRUE)[[1]]
    pb$tick()
    removal <- rollmax(removal, filter_window)
    type <- rollmax(type, filter_window)
    # The current numbers look off to me.
    # Have a flow path with adjacent segements with same type
    # Those should be only one segment...
    # maybe another filtering or another way to assign segments
    # Need to dig into this more.
    total_removal <- nsink_calc_total_removal(removal)
